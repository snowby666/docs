...
client = Client()
response = client.get("https://api.electronhub.ai/v1/user/me", headers=headers)
print(response.json())
```

```javascript Node.js
const response = await fetch('https://api.electronhub.ai/v1/user/me', {
  method: 'GET',
  headers: {
...
```bash cURL
curl -X GET "https://api.electronhub.ai/v1/user/me" \

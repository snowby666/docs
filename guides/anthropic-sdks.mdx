...
Point Claude Code at Electron Hub:

```json
{
  "env": {
    "ANTHROPIC_BASE_URL": "https://api.electronhub.ai",
    "ANTHROPIC_AUTH_TOKEN": "ek-your-api-key-here"
  }
}
```

Use base URL `https://api.electronhub.ai` (no `/v1` or `/v1/messages`). Coding Plan keys only allow `:dev` models; set `ANTHROPIC_MODEL` (and the `ANTHROPIC_DEFAULT_*_MODEL` overrides) to a Coding Plan model such as `kimi-k2.6:dev`. See [Messages API](/api-reference/messages).
...
<CardGroup cols={1}>
  <Card title="Prompt Caching Guide" icon="zap" href="/guides/prompt-caching">

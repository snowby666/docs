...
- **Cache writes**: 1.25x the original input pricing
- **Cache reads**: 0.1x the original input pricing

<Info>
Cache writes occur when content is first cached. Subsequent requests using the cached content are charged at the cache read rate (10% of original input price).
</Info>

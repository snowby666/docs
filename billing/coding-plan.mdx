...
| Series | Weight | Model ID | Context | Highlights |
|---|---|---|---|---|
| **Flash** | 0.5× | `gpt-oss-120b:dev` | 128K | High-reasoning MoE with strong tool use |
| **Flash** | 0.5× | `deepseek-v4-flash:dev` | 1M | Fast DeepSeek MoE (13B active); high-throughput coding & reasoning |
| **Flash** | 0.5× | `mimo-v2.5:dev` | 1M | Xiaomi omnimodal; agentic coding at low inference cost |
| **Standard** | 1× | `gemma-4-31b-it:dev` | 200K | Multimodal (text + image), 140+ languages |
| **Standard** | 1× | `qwen3.6-27b:dev` | 262K | Multimodal coding & reasoning, repo-level code comprehension |
| **Standard** | 1× | `hy3:dev` | 262K | Tencent MoE for reasoning, coding, and agent workflows |
| **Standard** | 1× | `minimax-m2.7:dev` | 180K | Agentic productivity and multi-agent collaboration |
| **Standard** | 1× | `kimi-k2.6:dev` | 240K | Long-horizon coding, UI/UX generation, agent swarms |
| **Heavy** | 2× | `glm-5.2:dev` | 200K | Repo-scale agentic software engineering |

All models support function calling and reasoning, on `/v1/chat/completions`, `/v1/messages`, and `/v1/responses`.

Series weight only affects soft full-speed headroom (`ceil(raw × weight)`), not whether you can keep using the plan. Dashboard per-model stats stay raw.
...
<AccordionGroup>
  ...
  <Accordion title="Full-speed soft headroom (daily)" icon="gauge">
    Each plan includes generous full-speed headroom that resets at **21:00 UTC**. Going past it never blocks you — requests continue with reduced concurrency and added latency until the reset. Your dashboard shows how close you are to full-speed headroom.
  </Accordion>
  <Accordion title="Full-speed soft headroom (weekly)" icon="calendar-week">
    A rolling weekly soft headroom sits above the daily one. Past it, the same slow lane applies (lower concurrency + latency) — not a hard stop. This keeps sustained 24/7 grinders from crowding out interactive work across the week.
  </Accordion>

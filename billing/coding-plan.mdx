...
description: 'Unlimited token usage for coding agents — flat monthly price, no credits, no per-token billing'
icon: 'terminal'
---

The **Electron Hub Coding Plan** (DevPass) is built for coding agents and CLI tools (Claude Code, Cline, Roo Code, Cursor, and anything OpenAI/Anthropic-compatible).
...
You get a dedicated `ek-dev-` API key that is fully independent of any other Electron Hub plan.
...
| Series | Model ID | Modality | Context · Quant | Notes |
|---|---|---|---|---|
| Flash · 0.5× | `gpt-oss-120b:dev` | <Icon src="/images/modality-text.svg" size={18} /> → <Icon src="/images/modality-text.svg" size={18} /> | 128K · MXFP4 | High-reasoning MoE, strong tool use |
| Flash · 0.5× | `deepseek-v4-flash:dev` | <Icon src="/images/modality-text.svg" size={18} /> → <Icon src="/images/modality-text.svg" size={18} /> | 1M · NVFP4 (FP4 MoE + FP8 attn) | Preview Flash; 1M context, high-throughput coding |
| Flash · 0.5× | `deepseek-v4-flash-0731:dev` | <Icon src="/images/modality-text.svg" size={18} /> → <Icon src="/images/modality-text.svg" size={18} /> | 400K · FP8 + FP4 (MXFP4 MoE) | Official V4 Flash GA; stronger agentic/coding (13B active) |
| Flash · 0.5× | `mimo-v2.5:dev` | <Icon src="/images/modality-text.svg" size={18} /> <Icon src="/images/modality-image.svg" size={18} /> → <Icon src="/images/modality-text.svg" size={18} /> | 1M · FP8 | Omnimodal agentic coding, low inference cost |
| Flash · 0.5× | `glm-4.7-flash:dev` | <Icon src="/images/modality-text.svg" size={18} /> → <Icon src="/images/modality-text.svg" size={18} /> | 200K · BF16 | Efficient 30B-class coding & tool use |
| Standard · 1× | `gemma-4-31b-it:dev` | <Icon src="/images/modality-text.svg" size={18} /> <Icon src="/images/modality-image.svg" size={18} /> → <Icon src="/images/modality-text.svg" size={18} /> | 200K · BF16 | Multimodal coding; 140+ languages |
...
| Standard · 1× | `minimax-m2.7:dev` | <Icon src="/images/modality-text.svg" size={18} /> → <Icon src="/images/modality-text.svg" size={18} /> | 180K · FP8 | Agentic productivity, multi-agent work |
...
| Heavy · 2× | `glm-5.2:dev` | <Icon src="/images/modality-text.svg" size={18} /> → <Icon src="/images/modality-text.svg" size={18} /> | 262K · FP8 | Repo-scale agentic software engineering |

All models support function calling on `/v1/chat/completions`, `/v1/messages`, and `/v1/responses`. Reasoning is available on every `:dev` model except `qwen3.6-27b:dev`.
...
Plans are for **individual coding**: one human working interactively in the tools they already use. Unlimited tokens, flat monthly price, and a key that is yours alone — every machine you own, never shared.

Token usage is never hard-capped. Elevated usage is slowed or reordered so interactive subscribers stay responsive when the platform is busy. Live status is always on the dashboard's Coding Plan tab.

...
<Info>
  Building with coding agents or CLI tools? There's a third option: the **[Coding Plan (DevPass)](/billing/coding-plan)** — **unlimited token usage** for **$10** (Lite) or **$40** (Turbo) per month, a dedicated `ek-dev-` key, and a curated set of `:dev` coding models. It never consumes credits and stacks with any other plan. Founding / beta subscribers keep their launch price while the subscription stays active.
</Info>

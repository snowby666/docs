...
- `message` &mdash; human-readable description (always present).
- `type` &mdash; error category (e.g. `invalid_request_error`).
- `param` &mdash; the offending parameter, or `null`.
- `code` &mdash; the HTTP status code, or `null`.

...
`ek-dev-` keys are **not** rate-limited in RPM at all. They use a concurrency model instead (2 simultaneous requests on Lite, 5 on Turbo) plus a fair-use queue — see the [Coding Plan docs](/billing/coding-plan#how-fair-use-works) for how admission, the daily allowance, and priority dispatch work.
